...
def det(image, image_id, pic):
    ...
    res = output['conv20'][0]  # the output probability vector for the first image in the batch
    print(res)
    ...
    for h in range(13):
            for w in range(13):
                for c in range(50):
                    swap[h*13+w][c/10][c%10]  = res[c][h][w]
