...
void RegionLossLayer<Dtype>::Forward_cpu(
		...
		for (int j = 0; j < side_; ++j) {
			for (int i = 0; i < side_; ++i) {
				for (int n = 0; n < num_; ++n) {
					...
					/*if (best_iou > 1) {
						LOG(INFO) << "best_iou>1"; // if it happend , plz tell me
						diff[index + 4] = (-1) * object_scale_ * (1 - swap_data[index + 4]) *logistic_gradient(swap_data[index + 4]);
						//delta_region_class(swap_data, diff, index + 5, class_label, num_class_, class_scale_, &avg_cat); //softmax_tree_
	...
	//LOG(INFO) << bottom[0]->count(1) << "," << bottom[0]->count();
